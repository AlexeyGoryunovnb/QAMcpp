...
public:
    void add_noise(double& x, double& y, double gamma) {
        double noise = std::sqrt((x*x+y*y) / (2 * gamma));
...
void plot_results(const std::vector<double>& SNR_db, const std::vector<double>& pe, int p) {
    ...
    plotfile << "plot 'qam_results.dat' with linespoints title 'Simulated BER', \\\n";

    if (p == 0) {

        plotfile << "     0.5*erfc(sqrt(10**(x/10)/2)) title 'Theoretical BER (p=0)'\n";
    }
    else {
        int q = 1 << ((p + 1) * 2);
        
        //plotfile << "     (sqrt(" << q << ")-1)/" << q << "*erfc(sqrt(3*10**(x/10)/" << q-1 << "/2))*(sqrt(" << q << ")-(sqrt(" << q << ")-1)*erfc(sqrt(3*10**(x/10)/" << q-1 << "/2))/2) title 'Theoretical Upper Bound', \\\n";
        plotfile << "     (sqrt(" << q << ")-1)/" << q << "*erfc(sqrt(3*10**(x/10)/" << q - 1 << "/2))*(sqrt(" << q << ")-(sqrt(" << q << ")-1)*erfc(sqrt(3*10**(x/10)/" << q - 1 << "/2))/2)/" << p + 1 << " title 'Theoretical BER', \\\n";
    }
    plotfile.close();
}
...
int main() {
    ...
    for (double g = -15.0; g <= 21.0; g += 0.3) {
        SNR_db.push_back(g);
    }
